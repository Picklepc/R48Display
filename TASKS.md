...
- [ ] **M5.0-05** THE PAYOFF (in progress): **CONFIRMED** — `setBufferSizes()` does NOT exist on this core's secure client, and the pinned 3.1 sdkconfig ships neither dynamic nor asymmetric mbedTLS buffers, so being on 3.x alone does not shrink the TLS footprint. The real (pioarduino-only) path:
  ...
  - [x] **M5.0-05d** ✅ (a22) Implemented, no `custom_sdkconfig`: the check AND apply tear BLE down first (`fwUpdPauseBle`, freeing ~40 KB) before the TLS session and restore it after; the check logs `[fwupd] check: internal free/largest`. `_updates` env + `updates-build.yml` removed. **Hardware test = M5.0-05c** (does the handshake fit on core 3, and does the web stay stable?).
  - [x] **M5.0-05b** ✅ (a22) `fwUpdTask` started (16 KB stack, core 0); `on_device`=true; `/api/update/check` + `/apply` live. Auto-checks default **OFF** — user-initiated lookups only.
...
- [x] **M5.0-11** ✅ (a22) Settings → Firmware Update: user-initiated **Check for updates** (no auto). Shows "installs on device" for a same-generation OTA, or "major update — install over USB" when the latest crosses the bootloader era (`requires_full_flash` from the check). Hours are persisted before an OTA reboots (maintenance-critical). Final confirmation is gated on M5.0-05c (the check actually fitting on hardware).
