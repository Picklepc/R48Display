...
- [ ] **M5.0-05** THE PAYOFF (in progress): **CONFIRMED** — `setBufferSizes()` does NOT exist on this core's secure client, and the pinned 3.1 sdkconfig ships neither dynamic nor asymmetric mbedTLS buffers, so being on 3.x alone does not shrink the TLS footprint. The real (pioarduino-only) path:
  - [ ] **M5.0-05a** `platformio.ini` → `custom_sdkconfig = CONFIG_MBEDTLS_DYNAMIC_BUFFER=y` (+ `CONFIG_MBEDTLS_ASYMMETRIC_CONTENT_LEN=y`, smaller `OUT_CONTENT_LEN`) so the ~16 KB TLS buffers are freed when idle instead of reserved. Triggers a full IDF-lib rebuild. **BLOCKED locally (2026-07-12):** the hybrid IDF build rejects the space in the project path (`C:\Users\Roto Router\…` → *"Detected a whitespace character in project paths"*). Options: (i) build the update-enabled variant in **CI only** (runner paths have no spaces); (ii) relocate the project + `.platformio` to a space-free path (e.g. `C:\dev\R48Display`); (iii) drop on-device TLS updates and stay USB-installer-only. **Update (2026-07-13):** tried (i) via the `_updates` env + `updates-build.yml` — the CI hybrid build ALSO fails: it rebuilds the IDF components and dies on a missing embedded cert (`https_server.crt.S not found`, from esp_https_server/esp_insights). So `custom_sdkconfig` is blocked both ways. **Preferred pivot → M5.0-05d.**
  - [ ] **M5.0-05d** Try the memory fix **without** `custom_sdkconfig`: enable `fwUpdTask`, and have the check/apply **tear BLE down first** (`fwUpdPauseBle()` already frees the ~40 KB BLE controller — wired up for OTA in a20). Then hardware-test whether the TLS handshake fits in that freed internal RAM on core 3 (2.x couldn't, but 3.x has a better allocator + a clean freed block). If it fits, on-device updates work with no IDF rebuild. Capture free-heap/largest-block with BLE down.
  - [ ] **M5.0-05b** Start `fwUpdTask` (`xTaskCreate`, 16 KB stack, core 0) so `on_device`=true and `/api/update/check` + `/apply` work again.
  - [ ] **M5.0-05c** Hardware-verify: capture free-heap/largest-block before & after a check; confirm the check + version list + self-update all succeed AND the web UI stays stable (the 0.4.10 fragmentation regression must not return).
...
- [x] **M5.0-07** Bootloader/partition: 3.x ships an IDF 5.x bootloader. OTA replaces only the app slot, NOT the bootloader, so the FIRST 3.x install should be a USB merged flash (bootloader+partitions+app). Fresh-install boot loops were traced to release packaging forcing the merged image bootloader header to QIO while PlatformIO direct USB upload writes DIO for ESP32-S3. v0.5.0a15 packaging now merges with DIO and CI rejects non-DIO merged images. Partition **table** is unchanged (ours, in `partitions.csv`), so no repartition.
...
- [x] **M5.0-09** ✅ No app-only flash across a bootloader boundary. The web installer always flashes the merged image at offset 0 (inherently complete). `firmwareEra()` maps a version to its arduino-esp32 generation (2 = ≤0.4.x, 3 = ≥0.5.0); `apiUpdateApply` refuses (`409 requires_full_flash`, points to the USB installer) when the target crosses the running firmware's era — fail-open if either version is unparseable so same-era updates never break. Manual-upload UI warns that cross-generation moves must use the USB installer. (The current 2.x→3.x boundary is already behind core-3 devices; this guards the *next* boundary too.)
- [x] **M5.0-10** ✅ Full-device backup & restore (board migration). **Decision:** raw **NVS-partition image** (byte-for-byte 256 KB blob), not selective JSON — user wants a complete clone including all hours history, and accepted that it contains Wi-Fi/MQTT/AP/OTA passwords **in plain text** (UI must warn). `GET /api/backup` streams the image; `POST /api/backup/restore` buffers it in PSRAM, writes it over the `nvs` partition, and reboots (a bad/partial write just resets NVS to defaults on boot — not brickable). Done (a21): backend + Settings-page **Backup &amp; Restore** card (download button with a plaintext-password warning; restore upload behind an "overwrites everything and reboots" confirm). Enables migrating history to a replacement board when one fails.
- [ ] **M5.0-11** On-device **stable firmware lookup + upgrade from the Settings page**, shipped in 0.5.0. Settings → Firmware Update looks up the latest **stable** release (skips pre-releases, like the installer's `/releases/latest`) and offers a one-click upgrade. This is the front-end for M5.0-05: wire the existing `/api/update/check` + `/api/update/apply` + version picker to the stable channel, and re-show the update UI that a11's `renderUpdateStatus()` hides while `on_device` is false. Depends on M5.0-05a (TLS memory) landing first.
- [x] **M5.0-13** ✅ Reliable OTA on a live device (a20): the ArduinoOTA/espota path now tears BLE down for the transfer (`fwUpdPauseBle()` in `onStart`, resume on `onError`), mirroring the on-device updater. Previously BLE + OTA contended for radio + internal RAM and the upload dropped mid-stream (`WinError 10054` ~50–70%). Takes effect for OTAs performed *after* a20 is installed; the a18→a19/a20 hop must be USB.
- [ ] **M5.0-12** **Revert to the last working firmware when an upgrade fails.** A bad OTA image (won't boot, crashes, or hangs before setup completes) must roll back to the previous firmware. Mechanism confirmed present on the pinned 3.1 core: `CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE=y`, `verifyRollbackLater()` defers validation, and `esp_ota_mark_app_valid_cancel_rollback()` confirms only after a healthy boot — so an unconfirmed image is auto-reverted by the bootloader. Validate end-to-end on 0.5.0: flash a deliberately-bad image and confirm the device returns to the working one.
