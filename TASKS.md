...
- [ ] **M5.0-10** Settings backup & restore: export the device's NVS-backed config (Wi-Fi, BMS profile, hours, pay, maintenance, theme, power settings, etc.) to a downloadable file from the web UI, and import/restore it — applying only the keys the running firmware understands (schema-tolerant / version-compatible). Lets users preserve their config across a full USB flash or a partition change that wipes NVS.
