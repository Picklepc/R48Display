...
- [ ] **M5.0-09** Firmware tool installs **complete** images (bootloader + partitions + app) for any release that changes the bootloader or partition table — no app-only path for those. The web installer already flashes the merged image at offset 0; the on-device OTA updater (app-only) must **refuse** such releases and send the user to the USB installer. Mechanism: mark releases that require a full flash (a `requires_full_flash` / minimum-bootloader field in the release metadata / `versions.json`) so both the installer UI and the on-device updater honor it and never offer app-only across a bootloader/partition boundary.
- [~] **M5.0-10** Full-device backup & restore (board migration). **Decision:** raw **NVS-partition image** (byte-for-byte 256 KB blob), not selective JSON — user wants a complete clone including all hours history, and accepted that it contains Wi-Fi/MQTT/AP/OTA passwords **in plain text** (UI must warn). `GET /api/backup` streams the image; `POST /api/backup/restore` buffers it in PSRAM, writes it over the `nvs` partition, and reboots (a bad/partial write just resets NVS to defaults on boot — not brickable). Backend done (a21); **TODO: Settings-page UI** (download button + plaintext-password warning; restore upload + "overwrites everything and reboots" warning). Enables migrating history to a replacement board when one fails.
