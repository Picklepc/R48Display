...
String settingsBody() {
  return F(
      ...
      "<label class='check'><input name='feature_mic' type='checkbox'> Audio assist (mic enabled)</label>"
      "<label>Mic Threshold (RMS)<input name='mic_run_threshold' type='number' min='100' max='12000' step='50'><span class='hint'>Tune using the MIC value on the LVGL dashboard while mowing</span></label>"
      "<label>Work Trigger<select name='work_trigger'>"
      "<option value='amps'>Amps only (default)</option>"
      "<option value='mic'>Mic only</option>"
      "<option value='either'>Amps or Mic (either)</option>"
      "</select></label>"
      "</div>"
      "</div>"
