...
- Cross-origin request guard on all state-changing API endpoints: browser requests with a mismatched `Origin` header are rejected with 403. Non-browser clients (curl, scripts, Home Assistant) are unaffected, and the `/update` recovery flow is deliberately left open
